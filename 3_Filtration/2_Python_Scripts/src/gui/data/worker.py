            ...
            if p.run_phase_0:
                ...
                if p.phase_0_mode == "continuous":
                    ...
                    while not self._should_abort():
                        ...
                        # Raw flow entpacken
                        if isinstance(flow_raw, tuple): flow_raw = flow_raw[-1]
...
# --- PHASE A: RAMP UP ---
            if p.run_phase_a:
                ...
                if p.phase_a_mode == "manual":
                    ...
                    # Manual: Bleibt eine Treppe, wartet auf Klick
                    ...
                    self.log_msg.emit(f"Phase A: Continuous Auto Ramp ({p.phase_a_rate_mbar_min} mbar/min).", "#8B5CF6")
                    
                    # 🚀 FIX: Berechne die Dauer aus Ziel und Rate
                    rate_mbar_s = float(p.phase_a_rate_mbar_min) / 60.0
                    if rate_mbar_s > 0:
                        total_duration_s = float(p.phase_a_target_mbar) / rate_mbar_s
                    else:
...
class Experimentator:
    ...
    def _get_pressure_setpoint_pct(self, channel: int) -> Optional[float]:
        try: return self._unwrap_sensor(self.dev.get_pressure_setpoint(int(channel)))
        except Exception: return None

    def _get_pressure_meas_pct(self, channel: int) -> Optional[float]:
        try: return self._unwrap_sensor(self.dev.get_pressure(int(channel)))
        except Exception: return None

    def _get_pressure_setpoint_mbar_best(self, channel: int) -> Optional[float]:
        try:
            if hasattr(self.dev, "get_pressure_setpoint_mbar"): return self._unwrap_sensor(self.dev.get_pressure_setpoint_mbar(int(channel))) # type: ignore[attr-defined]
        except Exception: pass
    ...
    def _get_pressure_meas_mbar_best(self, channel: int) -> Optional[float]:
        try:
            if hasattr(self.dev, "get_pressure_mbar"): return self._unwrap_sensor(self.dev.get_pressure_mbar(int(channel))) # type: ignore[attr-defined]
        except Exception: pass
    ...
    def _safety_check(self, mode: str) -> None:
        ...
        if margin < 0:
            robust_switch_valves(self.dev, "SHUT")
            raise RuntimeError(f"SAFETY STOP: volume {self.volume_ml:.6f} mL below min {self.cfg.min_volume_ml:.6f} mL (mode={mode})")
    ...
    def _log_row(self, mode: str, dt_s: float, flow_raw: float, flow_ml_s: float, *, net_flow_ml_s: float = 0.0, pressure_channel: Optional[int] = None, event: str = "") -> None:
        ...
        try: 
            if hasattr(self.dev, "get_valve_state"): valve_state = str(self.dev.get_valve_state())
    ...
    def step_staircase_ramp(self, *, target_pressure_mbar: float, step_size_mbar: float, step_time_s: float, wait_for_ok_fn: Optional[Callable[[str], None]] = None) -> float:
        ch = int(self.cfg.main_pressure_channel)
        robust_switch_valves(self.dev, "FILTRATION")
    ...
    def step_continuous_ramp(self, *, target_pressure_mbar: float, duration_s: float, abort_check_fn: Optional[Callable[[], bool]] = None) -> float:
        ch = int(self.cfg.main_pressure_channel)
        robust_switch_valves(self.dev, "FILTRATION")
        ...
        self._log_row(mode, 0.0, float("nan"), float("nan"), pressure_channel=ch, event=f"START_RAMP to {target_pressure_mbar}mbar")
        ...
        while True:
            if abort_check_fn and abort_check_fn(): break
            now = time.monotonic()
            if now >= t_end: break
