    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.MouseMove:
            if self.main_window and hasattr(
                    self.main_window, 'hud') and self.main_window.hud.isVisible():
        ...
        key_fn = getattr(event, "key", lambda: None)
        if (event.type() == QEvent.Type.KeyPress
                and key_fn() == Qt.Key.Key_Space
...
class CustomTitleBar(Qtw.QFrame):
    def __init__(self, parent=None):
        ...
        btn_style = (
            ...
            "QPushButton:hover { background: #111827; color: #F8FAFC; }")
        close_style = (
            "QPushButton { background: transparent; border: none; "
            "color: #64748B; font-size: 16px; } "
            "QPushButton:hover { background: #FF1744; color: #F8FAFC; }")
...
class LiquidPelicanWidget(Qtw.QWidget):
    def __init__(self, parent=None):
        ...
        svg_empty = (
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 150">'
            f'<g stroke="#1E293B" stroke-width="2" fill="none" stroke-linejoin="round">'
            f'{polygons}</g></svg>'
        )
        svg_full = (
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 150">'
            ...
            '</linearGradient></defs>'
            f'<g stroke="#00E5FF" stroke-width="2.5" fill="url(#g)" stroke-linejoin="round">'
            f'{polygons}</g></svg>'
        )
        svg_mask = (
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 150">'
            f'<g stroke="#FFFFFF" stroke-width="2.5" fill="#FFFFFF" stroke-linejoin="round">'
            f'{polygons}</g></svg>'
        )

        self.pix_empty = QPixmap(self.size())
        self.pix_empty.fill(Qt.GlobalColor.transparent)
        QSvgRenderer(QByteArray(svg_empty.encode())).render(QPainter(self.pix_empty))

        self.pix_full = QPixmap(self.size())
        self.pix_full.fill(Qt.GlobalColor.transparent)
        QSvgRenderer(QByteArray(svg_full.encode())).render(QPainter(self.pix_full))

        self.pix_mask = QPixmap(self.size())
        self.pix_mask.fill(Qt.GlobalColor.transparent)
    ...
    def update_physics(self):
        self._phase += 0.1
        if self._fill_lvl <= 0.01 or not self.isVisible():
            return
            ...
            for _ in range(spawn_rate):
                if random.random() < 0.5:
                    self.particles.append(
                        Particle(
                            random.uniform(
                                60, 440), random.uniform(
                                y_base + 30, self.height() - 20)))

        alive = []
        for p in self.particles:
            p.x = p.start_x + math.sin(p.phase + p.y * p.speed) * 10 + p.drift
            p.y += p.vy
            if p.y > y_base + math.sin((p.x * 0.02) + self._phase) * 10:
                alive.append(p)
    ...
    def paintEvent(self, event):
        ...
        part_layer.fill(Qt.GlobalColor.transparent)
...
class PelicanHUD(Qtw.QWidget):
    def __init__(self, parent=None):
        ...
        self.box.setStyleSheet(
            "QFrame { background-color: rgba(5, 9, 20, 230); "
            "border: 2px solid rgba(236, 72, 153, 60); border-radius: 12px; }")
        ...
        tel_style = (
            "color: #0284C7; font-family: 'Consolas'; font-size: 10px; "
        ...
        self.title.setStyleSheet(
            "color: #F8FAFC; font-family: 'Consolas', monospace; font-size: 32px; "
            "font-weight: 900; letter-spacing: 12px; background: transparent; border: none;")
        ...
        self.bar.setStyleSheet(
            ...
            "x1:0, y1:0, x2:1, y2:0, "
            "stop:0 #EC4899, stop:0.4 #8B5CF6, stop:0.7 #00E5FF, stop:1 #00FF66); }")

        self.status = Qtw.QLabel("")
        self.status.setStyleSheet(
            "color: #00E5FF; font-family: 'Consolas'; font-size: 13px; font-weight: bold; "
    ...
    def resizeEvent(self, event):
        ...
        self.box.move(int((self.width() - self.box.width()) / 2),
                      int((self.height() - self.box.height()) / 2))
    ...
    def _master_tick(self):
        ...
        if w > 100 and h > 100:
            self.box.move(int((w - self.box.width()) / 2 + self.current_dx),
                          int((h - self.box.height()) / 2 + self.current_dy))
...
class MainWindow(Qtw.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            if self.statusBar() is not None:
                self.statusBar().setStyleSheet(
                    ...
                    "font-family: 'Consolas'; font-weight: bold; border: none; font-size: 11px; }")
        except Exception:
            pass
        ...
        if not isinstance(health_cfg, dict):
            health_cfg = {}
        alarm_default = float(
            health_cfg.get("pressure_alarm_mbar") or self.config.get(
                "pressure_alarm_mbar", 8000.0))

        val_idle = _first_present(
    ...
    def _on_valve_command(self, mode: str):
        ...
        (auch während Experiment möglich)."""
        dev = self.dev
        if dev is None:
    ...
    def _on_relay_toggle(self, relay_num: int, target_on: bool):
        ...
        try:
            ...
            if vc is not None and hasattr(vc, "relais"):
                ...
                vc._state = vc._state.__class__("UNKNOWN") if hasattr(
                    vc._state, '__class__') else "UNKNOWN"
    ...
    def _force_safe_state_now(self) -> None:
        ...
        with self._hw_mutex:
            try:
                self._dev_set_pressure_setpoint_best_effort(
                    channel=MAIN_CH, value_mbar=0.0, ramp=False)
            except Exception:
                pass
            try:
                self._dev_set_pressure_setpoint_best_effort(
                    channel=BACKWASH_CH, value_mbar=0.0, ramp=False)
            except Exception:
                pass
    ...
    def _manual_reason_hint(self) -> Tuple[bool, str, str]:
        ...
        if self._hold_active:
            return True, "ACTIVE", "Release SPACE to stop HOLD."
        if allowed:
            if self._experiment_running():
                return True, "Allowed", "Hold SPACE to manually run backwash."
            return True, "Allowed", "Hold SPACE to manually control backwash (idle)."
        if self._experiment_running():
            return False, "Blocked", "HOLD only allowed in BACKWASH_HOLD step."
        return True, "Allowed", "Hold SPACE to manually control backwash (idle)."
    ...
    def _render_manual_state(self, *, reason: str = "") -> None:
        ...
        try:
            ...
            self.left.set_hold_affordance(bool(allowed),
                                          str(hint if not active else "MANUAL HOLD ACTIVE"))
        except Exception:
            pass
    ...
    def _slope_over_window(self, hist: Deque[Tuple[float, float]],
    ...
    def _update_health_banner(self, *, new_run_started: bool = False) -> None:
        ...
        if dev is not None:
            try:
                p1 = _safe_float(_unwrap_sensor(dev.get_pressure_mbar(MAIN_CH)))
            except Exception:
                pass
            try:
                p2 = _safe_float(_unwrap_sensor(dev.get_pressure_mbar(BACKWASH_CH)))
            except Exception:
                pass
            try:
                p1_set = _safe_float(dev.get_pressure_setpoint_mbar(MAIN_CH))
            except Exception:
                pass
            try:
                ...
                pass
            try:
                q = _safe_float(_unwrap_sensor(dev.read_flow()))
            except Exception:
                pass
    ...
    def _ui_backwash_pressure_mbar(self) -> float:
        try:
            return float(max(0.0, self.left.sp_hold_p.value()))
        except Exception:
            return 0.0
    ...
    def _dev_set_valve_state(self, state: str) -> None:
        if self._simulation_mode:
            return
        if self.dev is None:
            raise RuntimeError("DeviceManager not available")
    ...
    def _dev_set_pressure_setpoint(
            self, *, channel: int, value_mbar: float, ramp: bool = True) -> None:
        if self._simulation_mode:
            return
        if self.dev is None:
            raise RuntimeError("DeviceManager not available")
        ...
        if callable(fn):
            fn(channel=int(channel), setpoint_mbar=float(value_mbar), ramp=bool(ramp))

    def _dev_set_pressure_setpoint_best_effort(
            self, *, channel: int, value_mbar: float, ramp: bool = True) -> int:
        if self._simulation_mode:
            return channel
        try:
    ...
    def _hold_source_set(self, source: str, active: bool) -> None:
        if str(source).lower() not in ("space", "mouse"):
            return
    ...
    def _bg_hw_hold_start(self, p: float):
        with self._hw_mutex:
            ...
            if dev is not None:
                try:
                    if hasattr(dev, "all_valves_shut"):
                        dev.all_valves_shut()
                    if hasattr(dev, "valves_backwash"):
                        ...
                        logger.error("HW ERROR: No backwash method found on device")
                except Exception as e:
    ...
    def _bg_hw_hold_stop(self):
        with self._hw_mutex:
            ...
            if dev is not None:
                try:
                    if hasattr(dev, "set_valve_state"):
                        dev.set_valve_state("ALL_SHUT")
                    elif hasattr(dev, "all_shut"):
                        dev.all_shut()
    ...
    def _poll_realtime(self) -> None:
        ...
        dev = getattr(self, 'dev', None)
        p1_set = dev.get_pressure_setpoint_mbar(1) if dev and hasattr(
            dev, "get_pressure_setpoint_mbar") else 0.0
        p2_set = dev.get_pressure_setpoint_mbar(2) if dev and hasattr(
            dev, "get_pressure_setpoint_mbar") else 0.0
