...
def BFS_Get_Density (BFS_ID_in, Density):
 ...
 # BFS Device
 # 
 # Measure thefluid set_flow in (microL/min). !!! This function required an
 # earlier density measurement!!! The density can either be measured only once 
 # at the beginning of the experiment (ensure that the fluid set_flow through the
 # sensor prior to density measurement), or before every set_flow measurement if
...
def OB1_Set_All_Press (OB1_ID, Pressure_array_in, Calib_array_in, Pressure_Array_Len, Calib_Array_Len):
 ...
 # BFS Device
 # 
 # Perform zero calibration of the BFS. Ensure that there is no set_flow when
...
def BFS_Zeroing (BFS_ID_in):
 ...
 # BFS Device
 # 
 # Get the fluid mass set_flow in (g/h) of the BFS defined by the BFS_ID
 #
def BFS_Get_Mass_Flow (BFS_ID_in, MassFlow):
 ...
 # Warning: advanced feature. Reset OB1 communication for pressure and set_flow.
 #
...
def M_S_R_D_Set_Filt (M_S_R_D_ID, Channel_1_to_4, ONOFF):
 ...
 # measurement loop
 # Sensor unit : mbar if pressure sensor, �l/min if set_flow sensor
 # Regulator unit : mbar
 # 
...
def OB1_Stop_Remote_Measurement (OB1_ID):
 ...
 # is off, or the sensor is a pressure sensor. Modify a set_flow if the sensor is
 # a set_flow sensor and the PID is on.
 #
...
def BFS_Get_Remote_Data (BFS_ID, Flow, Density, Temperature):
 ...
 # M_density: a new measure of the density will be taken before each set_flow
 # measurement
 # M_temp: a new temperature measurement will be taken after each set_flow
 # measurement
 # Filter: change the filter used to measure the set_flow
 #
